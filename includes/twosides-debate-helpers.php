...
// @id A1
add_action( 'add_meta_boxes', 'twosides_debate_add_comment_meta_box' );  
// @id A2
add_action( 'edit_comment', 'twosides_debate_save_custom_meta_box', 10, 3 ); 
 ...
 * @since 1.0.2
 */
function twosides_debate_add_comment_meta_box()
{
    add_meta_box(
        'twosides_comment_meta_box', 
        __('Twosides Data'), 
        'twosides_debate_metabox_callback', 
        'comment', 
        'normal', 
        'high', 
        null
 ...
 */
function twosides_debate_metabox_callback($comment) 
{    
    global $comment;
    $commtype = get_comment_meta($comment->comment_ID, 'twosides_commtype', true );
    wp_nonce_field(basename(__FILE__), "twosides-debate-meta-box-nonce"); 
    ?>
            <table class="form-table editcomment comment_xtra">
            <tbody>
            <tr>
            <td class="first"><?php esc_html_e( 'Comment Classification:', 'twosides-debate' ); ?></td>
 ...
 * @id A2
 */
function twosides_debate_save_custom_meta_box($comment_id)
{
    if (!isset($_POST["twosides-debate-meta-box-nonce"]) || 
    !wp_verify_nonce($_POST["twosides-debate-meta-box-nonce"], basename(__FILE__)))
        return $comment_id;
